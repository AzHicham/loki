...
impl TransitData {
    ...
    pub fn is_upstream_in_mission(&self,
        upstream : & Position,
        downstream : & Position,
        mission : & Mission,
    ) -> bool {
    ...
    pub fn next_position_in_mission(&self,
        position : & Position,
        mission : & Mission,
    ) -> Option<Position> 
    {
    ...
    pub fn stop_at_position_in_mission(&self,
        position : & Position,
        mission : & Mission,
    ) -> Stop {
        let pattern = self.pattern(&mission.stop_pattern);
    ...
    pub fn arrival_time_of(&self, trip : & Trip, position : & Position) -> SecondsSinceDatasetStart {
        ...
        let seconds_in_day = self.pattern(pattern).arrival_time_at(timetable, vehicle, position);
    ...
    pub fn debark_time_of(&self, trip : & Trip, position : & Position) -> Option<SecondsSinceDatasetStart> {
        let pattern = &trip.mission.stop_pattern;
        let timetable = &trip.mission.timetable;
        let vehicle = & trip.vehicle;
        let has_seconds_in_day = self.pattern(pattern).debark_time_at(timetable, vehicle, position);
    ...
    pub fn board_time_of(&self, trip : & Trip, position : & Position) -> Option<SecondsSinceDatasetStart> {
        let pattern = &trip.mission.stop_pattern;
        let timetable = &trip.mission.timetable;
        let vehicle = & trip.vehicle;
        let has_seconds_in_day = self.pattern(pattern).board_time_at(timetable, vehicle, position);
        has_seconds_in_day.as_ref().map(|seconds_in_day| {
            let days = &trip.day;
            SecondsSinceDatasetStart::compose(days, &seconds_in_day)
        })
    ...
    pub fn earliest_trip_to_board_at(&self,
    ...
    fn earliest_vehicle_to_board(&self, 
        waiting_time : & SecondsSinceDatasetStart,
     ...
     ) -> Option<(Vehicle, DaysSinceDatasetStart,SecondsSinceDatasetStart)> 
     {
        ...
        let mut best_vehicle_day_and_its_arrival_time_at_next_position : Option<(Vehicle, DaysSinceDatasetStart, SecondsSinceDatasetStart)> = None;
        
        while waiting_time_in_day <= latest_board_time_in_day {
            
            let has_vehicle  = self.earliest_vehicle_to_board_in_day(&waiting_day, 
                &waiting_time_in_day, 
                timetable,
                pattern_data, 
                position
            ...
            if let Some(vehicle) = has_vehicle {
                let vehicle_arrival_time_in_day_at_next_stop = pattern_data.arrival_time_at(timetable, &vehicle, &next_position);
                let vehicle_arrival_time_at_next_stop = SecondsSinceDatasetStart::compose(&waiting_day, vehicle_arrival_time_in_day_at_next_stop);
                if let Some((_, _, best_arrival_time)) = & best_vehicle_day_and_its_arrival_time_at_next_position {
                    if vehicle_arrival_time_at_next_stop < *best_arrival_time {
                        best_vehicle_day_and_its_arrival_time_at_next_position = Some((vehicle, waiting_day, vehicle_arrival_time_at_next_stop));
                    }
                }
                else {
                    best_vehicle_day_and_its_arrival_time_at_next_position = Some((vehicle, waiting_day, vehicle_arrival_time_at_next_stop));
    ...
    fn earliest_vehicle_to_board_in_day(&self, 
