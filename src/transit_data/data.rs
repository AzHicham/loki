...
use super::time::{DaysSinceDatasetStart, PositiveDuration};
...
impl TransitData {
    ...
    pub fn nb_of_timetables(&self) -> usize {
        self.patterns
            .iter()
            .map(|pattern| pattern.nb_of_timetables())
            .sum()
    }
    ...
    pub fn nb_of_vehicles(&self) -> usize {
        self.patterns
            .iter()
            .map(|pattern| pattern.nb_of_vehicles())
            .sum()
    }
    ...
    pub fn write_response<Writer: std::fmt::Write>(
        &self,
        response: &Journey,
        transit_model: &TransitModel,
        writer: &mut Writer,
        ...
        writeln!(
            ...
            "Arrival : {}",
            self.calendar
                .to_pretty_string(&response.arrival_section.to_datetime)
        )?;
        ...
        for (transfer_section, _, _) in response.connections.iter() {
            ...
            transfer_duration = transfer_duration
                + self.stops_data[stop.idx].transfers[transfer.idx_in_stop_transfers]
                    .1
                    .clone();
        }
        ...
        writeln!(
            ...
            "Departure : {}",
            self.calendar
                .to_pretty_string(&response.departure_section.from_datetime)
        )?;
    ...
    fn write_vehicle_section<Writer: std::fmt::Write>(
        &self,
        ...
        let vehicle_journey_idx = self.patterns[pattern.idx]
            .vehicle_data(timetable, vehicle)
            .vehicle_journey_idx;
        ...
        let from_datetime = self
            .calendar
            .to_pretty_string(&vehicle_section.from_datetime);
        let to_datetime = self.calendar.to_pretty_string(&vehicle_section.to_datetime);
        writeln!(
            writer,
            "{} from {} at {} to {} at {} ",
            route_id, from_stop_id, from_datetime, to_stop_id, to_datetime
        )?;
        Ok(())
    }
}
