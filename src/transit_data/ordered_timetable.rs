...
impl StopPatternData {
    ...
    pub fn can_debark(&self, position: &Position) -> bool {
        ...
        match flow_direction {
            FlowDirection::BoardAndDebark | FlowDirection::DebarkOnly => true,
    ...
    pub fn can_board(&self, position: &Position) -> bool {
        let flow_direction = &self.flow_directions[position.idx];
        match flow_direction {
            FlowDirection::BoardAndDebark | FlowDirection::BoardOnly => true,
            FlowDirection::DebarkOnly => false,
        }
    }

    fn timetable_data<'a>(&'a self, timetable: &Timetable) -> &'a TimetableData {
        self.timetables.get(timetable.idx).unwrap_or_else(|| {
            panic!(format!(
    ...
    pub fn timetables(&self) -> TimetablesIter {
        (0..self.timetables.len()).map(|idx| Timetable { idx })
    }
    ...
    pub fn nb_of_vehicles(&self) -> usize {
        self.timetables
            .iter()
            .map(|timetable| timetable.nb_of_vehicles())
            .sum()
    }
    ...
    pub fn vehicles(&self, timetable: &Timetable) -> VehiclesIter {
        ...
        (0..nb_of_vehicles).map(|idx| Vehicle { idx })
    }

    pub fn debark_time_at(
        &self,
        timetable: &Timetable,
        vehicle: &Vehicle,
        position: &Position,
    ...
    pub fn earliest_filtered_vehicle_to_board_at<Filter>(
        &self,
        waiting_time: &Time,
        timetable: &Timetable,
        position: &Position,
...
impl TimetableData {
    ...
    fn vehicle_board_then_debark_times<'a>(
        &'a self,
        vehicle_idx: usize,
