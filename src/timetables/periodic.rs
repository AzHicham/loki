...
use crate::time::{Calendar, SecondsSinceDatasetUTCStart, SecondsSinceTimezonedDayStart, DaysSinceDatasetStart};
...
impl TimetablesTrait for PeriodicTimetables {
    ...
    fn earliest_trip_to_board_at(&self, 
        waiting_time : & SecondsSinceDatasetUTCStart, 
        mission : &Self::Mission, 
        position : & Self::Position
    ) -> Option<(Self::Trip, SecondsSinceDatasetUTCStart)> {
        
        let has_earliest_and_latest_board_time = self.timetables.earliest_and_latest_board_time(position);
        ...
        let decompositions = self.calendar.decompositions(waiting_time, 
            timezone, 
        ...
        best_vehicle_day_and_its_arrival_time_at_next_position.map(|(vehicle, day, arrival_time_at_next_stop)| {
            let trip = Trip {
                vehicle,
                day,
            };
    ...
    where
     ...
     {
        let days_pattern = self.days_patterns.get_or_insert(valid_dates, &self.calendar);
        let vehicle_data = VehicleData {
            days_pattern,
            vehicle_journey_idx,

        };
        let insert_error = self.timetables.insert(stop_flows, 
            board_debark_timezoned_times.iter().map(|&tuple| tuple.clone()) , 
            timezone.clone(), 
            vehicle_data);
