...
use crate::{time::{Calendar, PositiveDuration}, traits::{Indices, Input, NetworkStructure, Response, TimeQueries, TransitIters, TransitTypes}};
...
impl<Timetables : TimetablesTrait> 
TransitTypes
for
...
where Timetables : TimetablesTrait + for<'a> TimetablesIter<'a>
{
    ...
    fn stop_point_idx_to_stop(&self, stop_point_idx : & Idx<StopPoint>) -> Option<Self::Stop> {
        self.stop_point_idx_to_stop.get(stop_point_idx).map(|stop| stop.clone())
    }
...
for
TransitData<Timetables> 
where Timetables : TimetablesTrait 
{
...
where Timetables : TimetablesTrait 
{
    fn board_time_of(&self, trip : &Self::Trip, position  : & Self::Position) -> Option<crate::time::SecondsSinceDatasetUTCStart> {
        self.timetables.board_time_of(trip, position)
    }

    fn debark_time_of(&self, trip : &Self::Trip, position  : & Self::Position) -> Option<crate::time::SecondsSinceDatasetUTCStart> {
        self.timetables.debark_time_of(trip, position)
    }

    fn arrival_time_of(&self, trip : &Self::Trip, position  : & Self::Position) -> crate::time::SecondsSinceDatasetUTCStart {
...
where Timetables : TimetablesTrait 
{
    fn to_naive_datetime(&self, seconds : &crate::time::SecondsSinceDatasetUTCStart) -> chrono::NaiveDateTime {
