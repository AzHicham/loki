...
fn fill_journey(
    ...
    proto
        .sections
        .resize_with(1 + 3 * journey.nb_of_connections(), || Default::default());

    let first_section = &mut proto.sections[0];
    let section_id = format!("section_{}_{}", journey_id, 0);
    fill_public_transport_section(
        &journey.first_vehicle_section(transit_data),
        first_section,
        model,
        section_id,
    )?;
...
fn fill_transfer_section(
    ...
    let proto_origin = proto
        .origin
        .get_or_insert_with(|| navitia_proto::PtObject::default());
    fill_stop_point_pt_object(transfer_section.from_stop_point, proto_origin, model)?;
    let proto_destination = proto
        .destination
        .get_or_insert_with(|| navitia_proto::PtObject::default());
...
fn fill_public_transport_section(
    ...
    let proto_pt_display_info = proto
        .pt_display_informations
        .get_or_insert_with(|| navitia_proto::PtDisplayInfo::default());
    fill_pt_display_info(
        vehicle_section.vehicle_journey,
        proto_pt_display_info,
        model,
    )?;
    ...
    proto
        .stop_date_times
        .resize(nb_of_stop_times, navitia_proto::StopDateTime::default());
...
fn fill_stop_point_pt_object(
    ...
    proto.poi = None;
    let proto_stop_point = proto
        .stop_point
        .get_or_insert_with(|| navitia_proto::StopPoint::default());
...
fn fill_stop_point(
    ...
    let proto_stop_area = proto
        .stop_area
        .get_or_insert_with(|| navitia_proto::StopArea::default());
...
fn fill_pt_display_info(
    ...
    proto.color = line.color.as_ref().map(|color| format!("{}", color));
    ...
    let proto_uris = proto
        .uris
        .get_or_insert_with(|| navitia_proto::Uris::default());
    {
...
fn fill_shape_from_stop_points(
