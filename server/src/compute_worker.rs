...
fn solve_journeys(
    ...
    let comparator_type = if journey_request.criteria.is_some() {
        match journey_request.criteria() {
            ...
            navitia_proto::Criteria::Occupancy => ComparatorType::Occupancy,
        }
    } else {
...
fn make_schedule_request<'a>(
    ...
    let from_datetime = {
        ...
        loki::NaiveDateTime::from_timestamp_opt(timestamp, 0).with_context(|| {
            format!(
                "Invalid timestamp {}, cannot be converted to a datetime.",
                timestamp
            )
        })?
    };
